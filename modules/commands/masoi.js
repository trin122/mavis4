...
module.exports.EndGame = async function(api, event,Data,winner) {
  switch (winner) {
    case "Village": {
      ...
      return api.sendMessage('[⚜️]➜ Tổng Ngày Trôi Qua: ' + Days + '\x0aTổng Người Còn Sống: ' + All['length'] + '\x0a\x0a' + Objection,event.threadID);
    }
    case "Werewolf": {
        ...
        case 9: {
          var role = ["Dân Làng","Sói thường","Tiên Tri","Bảo Vệ","Sói thường","Dân Làng","Dân Làng","Dân Làng","Dân Làng"];
            for (let i = 0;i < values.player.length;i++) {
              ...
              api.sendMessage({ body:  '[⚜️]➜ Vai Trò Của Bạn Là: ' + is['Name'] + '\x0aChi Tiết: ' + is['Description'] + '\x0aThuộc Phe: ' + is['Type'] + ' !',attachment: createReadStream(join(__dirname, is.File))}, values.player[i].id,(ee,inf) => { if (ee) { LOI.push(values.player[i].id);} });
            }
