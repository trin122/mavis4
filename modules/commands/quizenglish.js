...
module.exports.handleReaction = ({ api, event, handleReaction }) => {
	...
	if (response == handleReaction.answer) api.sendMessage("[⚜️] Bạn trả lời đúng rồi đấy xD", event.threadID);
	else api.sendMessage("[⚜️] Bạn trả lời sai rồi :X", event.threadID);
...
module.exports.run = async ({  api, event, args }) => {
	...
	return api.sendMessage(`[⚜️] Đây là câu hỏi dành cho bạn:\n        ${decodeURIComponent(fetch.data.results[0].question)}\n\n   👍: True       😢: False`, event.threadID, async (err, info) => {
		...
		if (data.answerYet !== 1) {
			api.sendMessage(`[⚜️] Time out!! đáp án chính xác là ${fetch.data.results[0].correct_answer}`, event.threadID, info.messageID);
