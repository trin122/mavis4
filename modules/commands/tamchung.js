module.exports.config = {
    ...
    version: "2.0.0",
    hasPermssion: 0,
    credits: "Trung Kiên fix",
    description: "Tắm chung",
...
async function makeImage({ one, two }) {
    ...
    let batgiam_img = await jimp.read(__root + "/tam.png");
    let pathImg = __root + `/tam${one}_${two}.png`;
    ...
    batgiam_img.resize(901, 1300).composite(circleOne.resize(140, 140), 247, 243).composite(circleTwo.resize(150, 150), 382, 231);
...
module.exports.run = async function ({ event, api, args }) {
    ...
    if (!mention) return api.sendMessage("Vui lòng tag 1 người", threadID, messageID);
        ...
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "Bạn rất may mắn😄😄" + tag + '\n',
            mentions: [{
          tag: tag,
          id: mention
        }],
