...
module.exports.config = {
    ...
    commandCategory: "Hệ Thống", // Thuộc vào nhóm nào: system, other, game-sp, game-mp, random-img, edit-img, media, economy, ...
...
module.exports.handleEvent = async ({ api, event, Users, Threads, args }) => {
    ...
    mentionsKey.forEach(async (each) => {
        ...
        if(allAdmin.includes(each)) {
            ...
            api.sendMessage({body:`=== 『 TAG ADMINBOT 』 ===\n━━━━━━━━━━━━━━━━\n\n[👤] ➜ Người tag: ${userName}\n[👨‍👩‍👧‍👦] ➜ Box: ${(await Threads.getInfo(threadID)).threadName || "Tên nhóm không tồn tại"}\n[💬] ➜ Nội dung: ${body}\n[⏰] ➜ Time: ${moment().tz("Asia/Ho_Chi_Minh").format("HH:mm:ss - DD/MM/YYYY")}\n[⚜️] ➜ Reply tin nhắn ( Phản hồi ) lại người tag`, attachment: await downLoad(`https://graph.facebook.com/${event.senderID}/picture?height=720&width=720&access_token=6628568379%7Cc1e620fa708a1d5696fb991c1bde5662`, __dirname+'/cache/ảnh/12345.jpg')},each, (err, info) => {
                if(err) console.log(err)
                else {
                    global.client.handleReply.push({
                        name: this.config.name,
